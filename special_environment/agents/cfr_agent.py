...
class CFRAgent():
    ...
    def train(self):
        ...
        for player_id in range(self.env.player_num):
            self.env.reset()
            #print("ENVIRONMENT RESET! ", player_id, self.env.player_num, end='\n')
    ...
    def traverse_tree(self, probs, player_id):
        ...
        '''
        if self.env.is_over():
            #print("self env OVER in cfr agent traverse tree ", end='\n')
            payoff_initiate_bluff = self.env.get_payoffs()
            for x in payoff_initiate_bluff:
                if x < 0 :
                    x = 0 
            return payoff_initiate_bluff
        ...
        for action in legal_actions:
            ...
            self.env.step_back()
            #print("step back utan")

            state_utility += action_prob * utility
            action_utilities[action] = utility

        #print("out of for loop in traverse tree")
