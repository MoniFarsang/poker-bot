...
class LeducholdemEnv(Env):
    ...
    def __init__(self, config):
        ...
        self.state_shape = [121] #initially 36
    ...
    def _extract_state(self, state):
        ...
        hand = state['hand']
        obs = np.zeros(121)          #initially 36
        obs[self.card2index[hand]] = 1
        if public_card:
            obs[self.card2index[public_card]+4] = 1  # +1 (+3 helyett +4)
        obs[state['my_chips']+8] = 1                 # obs[state['my_chips']+6] helyett +8
        #print("state all chips: ", state['all_chips'][1])
        obs[state['all_chips'][1]+22] = 1
        obs[state['bigRound']+36] = 1                  # 0 to 2 (3 games max), 36 to 38
        obs[state['relativeChip']+38] = 1                # 84 possible values (42 in both direction) -> 38 + 84 -1 = 121
