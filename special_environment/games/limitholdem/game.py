...
class LimitholdemGame(object):
    ...
    def init_game(self, relativeChip=None, bigRound = 0 ):
        ...
        #added by me
        if relativeChip is None:
            self.relativeChip = 0
        else:
            self.relativeChip += relativeChip
                
        self.bigRound = bigRound
        #added by me end 
    ...
    def step(self, action):
        ...
        if self.is_over_small():
            ...
            self.init_game(payoffs[0], prevGamePlusOne)
    ...
    def is_over_small(self):
        ''' Check if a smallround is over

        Returns:
            (boolean): True if the smallround is over
        '''
    ...
    def is_over(self):
        ...
        alive_players = [1 if p.status in (PlayerStatus.ALIVE, PlayerStatus.ALLIN) else 0 for p in self.players]
        # If only one player is alive, the game is over.
        if sum(alive_players) == 1 and self.bigRound==self.bigRound_no:
            return True

        # If all rounds are finshed
        if self.round_counter >= 4 and self.bigRound==self.bigRound_no:
            return True
        return False
    ...
    def get_payoffs(self):
        ...
        for i in range(len(payoffs)):
            calc_payoff[i] = self.currentChips[i] + payoffs[i]
        if self.bigRound == self.bigRound_no :
            for i in range(len(calc_payoff)):
                if calc_payoff[i] < 0 :
                    calc_payoff[i] = 0 
        print(calc_payoff)
        return calc_payoff
